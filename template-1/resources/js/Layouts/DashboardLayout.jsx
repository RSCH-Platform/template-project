...
export default function AppLayout({ children }) {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-slate-100 dark:bg-slate-950 transition-colors duration-200">
            ...
            {/* Floating Impersonate Widget */}
            {auth?.is_impersonating && (
                <div className="fixed bottom-6 right-6 z-50">
                    <div className="rounded-2xl border border-white/20 bg-slate-900/40 dark:bg-slate-800/40 backdrop-blur-lg px-5 py-4 flex items-center gap-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] transform transition-all duration-300 hover:scale-105">
                        <div className="flex items-center gap-3 text-white">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center font-bold shadow-inner">
                                {auth.user?.name?.charAt(0)?.toUpperCase()}
                            </div>
                            <div>
                                <p className="text-sm font-bold flex items-center gap-2 text-white">
                                    <span className="animate-pulse w-2.5 h-2.5 rounded-full bg-danger-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]"></span>
                                    Mode Impersonate
                                </p>
                                <p className="text-xs text-slate-200 mt-0.5">
                                    Sebagai <span className="font-semibold">{auth.user?.name}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <a 
                            ...
                            className="whitespace-nowrap px-4 py-2 bg-danger-500/90 hover:bg-danger-500 text-white rounded-xl font-bold text-sm transition-all shadow-lg hover:shadow-danger-500/50 hover:-translate-y-0.5 border border-white/10 backdrop-blur-sm"
                        >
                            Akhiri Sesi
                        </a>
                    </div>
